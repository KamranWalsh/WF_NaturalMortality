...
my.title <- paste0("Catch-at-Age for GB NMFS Spring Survey")
  ...
  bubble.legend1 <- c(0.5,1.5,2.5)
  #bubble.legend2 <- bubble.legend1 * scale.catch.obs
  legend("topright", xpd=TRUE, legend=bubble.legend1, pch=rep(21, 3), pt.cex=bubble.legend1, horiz=T , col='black', pt.bg = bubble.col)
...
nyrs = length(years)
ages = c(0:7)
ages.lab = c(as.character(ages[0:7]), "7+")
n_ages = length(ages)
...
### put both in the same plot window 

#png("~/Desktop/StockAssessmentPracticum/Figures/PaperFigures/GOM_agecomps.png", width = 7, height = 7, units = "in", res = 300)

origpar <- par(no.readonly = TRUE)
par(mfrow=c(1, 2), mar=c(3, 1, 3, 0.5), oma=c(2, 4, 2, 1))
...
z3 <- as.matrix(acomp.obs)
scale.catch.obs <- 5

plot(ages, rev(ages), xlim = range(ages), ylim = c(years[nyrs],(years[1]-2)), xlab = "", ylab = "", type = "n", axes=FALSE, main="Spring", font.main = 1)
axis(1, at= ages, lab = ages.lab)
axis(2, at = rev(years), lab = rev(years), cex.axis=0.75, las=1)
box()
abline(h=years, col="lightgray")
segments(x0=ages, y0=rep(years[1],n_ages), x1=ages, y1=rep(years[nyrs],n_ages), col = "lightgray", lty = 1)
if (length(catch.yrs)>0) for (j in 1:nyrs) points(ages, rep(years[j], n_ages), cex=z3[j,], col="black", pch = 21)

bubble.legend1 <- c(0.5,1.5,2.5)
#bubble.legend2 <- bubble.legend1 * scale.catch.obs
legend("topright", xpd=TRUE, legend=bubble.legend1, pch=rep(21, 3), pt.cex=bubble.legend1, horiz=T , col='black', bty="n", inset=c(0, 0))
...
plot(ages, rev(ages), xlim = range(ages), ylim = c(years[nyrs],(years[1]-2)), xlab = "", ylab = "", type = "n", axes=FALSE, main="Fall", font.main = 1)
axis(1, at= ages, lab = ages.lab)
axis(2, at = rev(years), labels = FALSE, tick = FALSE)
box()
abline(h=years, col="lightgray")
segments(x0=ages, y0=rep(years[1],n_ages), x1=ages, y1=rep(years[nyrs],n_ages), col = "lightgray", lty = 1)
if (length(catch.yrs)>0) for (j in 1:nyrs) points(ages, rep(years[j], n_ages), cex=z3[j,], col="black", pch = 21)
...
mtext("Year", side = 2, outer = TRUE, line = 2.5, cex = 1.2)
mtext("NEFSC Bottom Trawl Survey GOM Abundance-at-Age", side = 3, outer = TRUE, line = 0, cex = 1.2, font = 2)
par(origpar)
...
#png("~/Desktop/StockAssessmentPracticum/Figures/PaperFigures/SNEMA_agecomps.png", width = 7, height = 7, units = "in", res = 300)
...
mtext("Age", side = 1, outer = TRUE, line = 0.5, cex = 1.2)
mtext("Year", side = 2, outer = TRUE, line = 2.5, cex = 1.2)
mtext("NEFSC Bottom Trawl Survey SNEMA Abundance-at-Age", side = 3, outer = TRUE, line = 0, cex = 1.2, font = 2)
par(origpar)
...
group_reg_allship <- function(data, target_var, other_vars, group_var) {
  ...
  for (var in other_vars) {
    ...
    #get rid of error in cases where p value is low but coef_val = NA because all values for a species/stock region/season combination are 0 
  ...
  if (grepl("winter_flounder_wt", target_var, ignore.case = TRUE)) { winterflounder_name <- "Winter Flounder kg/tow" }
      ...
      #           vjust = -0.7, size = 3, color = "black", inherit.aes = FALSE ) + 
        ...
        labs(
          subtitle = paste(" R² =", round(r_squared,3), "| p =", round(p_value, 3)),
          x = pred_label,
...
total_number_plot <- data.frame(
  NameofSpecies=total_number$species ,  
  NumberofSpecies= total_number$count,
  PredorComp = c("Predator", "Competitor", "Competitor", "Both", "Both", "Competitor") #this is still hard coded; find some way to automate this later
  ) %>% 
  mutate(NameofSpecies = recode(NameofSpecies,
                                "goosefish" = "monkfish",
                                "rosette_skate" = "rosette skate",
                                "thorny_skate" = "thorny skate",
                                "white_hake" = "white hake",
                                "witch_flounder" = "witch flounder"
                                )) #%>%
  #arrange(desc(NumberofSpecies), NameofSpecies) %>%
  #mutate(NameofSpecies = factor(NameofSpecies, levels = NameofSpecies))
...
total_number_plot <- data.frame(
  ...
  mutate(NameofSpecies = recode(NameofSpecies,
                                ...
                                "goosefish" = "Monkfish",
                                "longhorn_sculpin" = "longhorn sculpin",
                                "ocean_pout" = "ocean pout",
                                "red_hake" = "red hake",
                                "silver_hake" = "silver hake",
                                "spiny_dogfish" = "spiny dogfish", 
                                "white_hake" = "white hake",
                                "winter_skate" = "winter skate"
                                )) #%>%
  #arrange(desc(NumberofSpecies), NameofSpecies) %>%
  #mutate(NameofSpecies = factor(NameofSpecies, levels = NameofSpecies))
